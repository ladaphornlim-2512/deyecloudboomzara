...
await p.goto(BASE+"/history",{waitUntil:"networkidle2"}); await sleep(5000);
await p.evaluate(()=>{const h=[...document.querySelectorAll("h2,h3")].find(e=>e.textContent.includes("วิเคราะห์")); if(h)h.scrollIntoView({block:"start"});}); await sleep(800);
await p.screenshot({path:"design/shots/real-hist-day.png"}); console.log("shot day");
await b.close();
